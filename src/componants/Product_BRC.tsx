...
function Product_BRC({product}: {product: RealEstate}) {
  ...
  return (
      <div className="product w-full flex-col relative h-full mb-5 bg-white flex  border border-[#ccc] rounded-xl ">
...
{index > 0 &&                             <IoIosArrowBack onClick={() => setIndex((a) => a - 1)} size={30} className="cursor-pointer absolute h-full left-2 top-1/2 -translate-y-1/2 text-white" />
                         ...
                         {/* {user && <AddToFavourite isFavourite={user?.id && data?.map((e) => e?.id).includes(product?.id)} product={product} />} */}
                            ...
                            {product?.images.map((e,a) =>                         <div key={a} className={`w-2 h-2 bg-white rounded-full transition-all ${index === a && "w-3 h-3"}`} onClick={()  => setIndex(a)}></div>)}
                        ...
                        <div className="text p-7">
                            ...
                            <p className='text-[16px] text-gray-700'>{product?.description.slice(0,20)}</p>
                             ...
                             <div className="props flex gap-4 my-1.5">
                                                            ...
                                                            <div className="bed flex gap-2 items-center text-[#474747]">
                                                                <h1>{product?.Sqft}</h1>
                                                                <RxDimensions size={17} color="#474747" />
                                                            </div>
                                                        </div>
                        ...
                        <div className="boxes bg-gray-100 p-3 rounded-b flex items-center justify-end gap-3 ">
