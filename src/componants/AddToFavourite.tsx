...
function AddToFavourite({Type = true,product,isFavourite}:any) {
  ...
  return (
 <>
 {Type === "product_landing" ?  <div onClick={() => {AddToFavourite.mutate(product);setFav((e) => !e)}} className="favourite p-2 absolute top-5 left-5 cursor-pointer bg-white dark:!bg-gray-800  dark:text-white rounded-full z-1 text-black">
            ...
            </div>  : <Button onClick={() => {AddToFavourite.mutate(product);setFav((e) => !e)}} variant={"outline"} size={"icon-lg"} className='p-2 border cursor-pointer transition-all hover:bg-slate-50 border-gray-300 bg-white dark:!bg-gray-800  dark:!border-gray-600 rounded-lg '>
                                                                  {fav ?  <IoHeart  size={27} className='text-red-500' /> :  <IoHeartOutline  size={27} className='' />}
                                                                </Button>
                          }
 </>
