...
console.log(status)
        ...
        const test = await fetch(`/api/authUser/${!switcher || session?.isNewUser ? "register" : "login"}` , {
        method:"POST",
        headers:{
...
<div className="switch flex gap-1 p-1 w-full border font-medium cursor-pointer border-gray-300  dark:!bg-gray-800  dark:!border-gray-600  rounded-full mt-6  ">
    <span className={`w-1/2  rounded-full p-2.5 text-gray-700 dark:text-gray-200 text-center transition-all duration-500 ${switcher && "bg-primary text-white"}`} onClick={() => setSwitch(true)}>Log In</span>
    <span  className={`w-1/2  rounded-full p-2.5                     text-gray-700 dark:text-gray-200 text-center transition-all duration-500 ${!switcher && "bg-primary text-white"}`} onClick={() => {setSwitch(false); setWrong(false)}}>Register</span>
</div>

<form action="" onSubmit={handleSubmit((data) => mutation.mutate(data))} className='mt-6'>
   {!switcher &&  <div className="userName">
        <Label className={`text-sm font-medium `}>UserName</Label>
        <input {...register("userName" , {required:"UserName is requried"})} type="text" name='userName' id='userName' placeholder='Enter your userName' className='w-full mt-2 mb-4  dark:!bg-gray-800  dark:!border-gray-600 px-3 py-3 border border-gray-300 rounded-md outline-none text-sm focus:border-[#333446] transition-all duration-200 '/>
    </div>}
    <div className="email">
        <Label className={`text-sm font-medium  ${wrong ? " text-red-500" : ""}`}>Email</Label>
        <input onFocus={() => setWrong(false)} {...register("email" , {required:"Email is requried"})} type="email" name='email' id='email' placeholder='Enter your email' className={`${wrong ? "border border-red-500" : ""}  dark:!bg-gray-800  dark:!border-gray-600 w-full mt-2 mb-4 px-3 py-3 border border-gray-300 rounded-md outline-none text-sm focus:border-[#333446] transition-all duration-200 `}/>
    </div>
    <div className="password">
        <Label  className={`text-sm font-medium  ${wrong ? " text-red-500" : ""}`}>Password</Label>
        <div className={`input relative px-3 py-3 w-full mt-2 mb-4  border border-gray-300  dark:!bg-gray-800  dark:!border-gray-600  rounded-md outline-none text-sm group-focus:border-[#333446] transition-all duration-200  ${wrong ? "border border-red-500" : ""}`}>
                <input onFocus={() => setWrong(false)} type={showPassword ? "text" : "password"} {...register("password" , {required:"Password is requried"})} name='password' id='password' placeholder='Enter your password' className={`w-full group outline-none`} />
{showPassword ? (
                <EyeOff onClick={() => setShowPassword((e) => !e)} className='absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 cursor-pointer' />
) : (
                <Eye onClick={() => setShowPassword((e) => !e)} className='absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 cursor-pointer' />
)}
        </div>
        </div>
        ...
        <div className="links flex justify-between items-center mt-4">
            <div className="remember flex items-center">
                ...
                <Label className='text-sm !font-medium text-gray-800 ml-2 dark:text-gray-400'>Remember me</Label>
            </div>
...
<div className="buttons gap-4 mt-4">
    <button onClick={() => signIn("google").then((e) => e?.ok )} className='w-full justify-center rounded-full mt-6 dark:!borer-none bg-[#f6f5f8] px-3 border border-gray-200 text-gray-800 py-3.5 md:text-lg  cursor-pointer hover:bg-[#e0e0e0] transition-all duration-200 font-semibold flex items-center  gap-5'>
        <FcGoogle size={30} />
       <span className='text-center'> Continue with Google</span>
    </button>
    <button onClick={() => signIn("facebook").then((e) => e?.ok )} className='w-full justify-center bg-[#3a5187] dark:border-none rounded-full mt-6 text-white px-3 border border-gray-200  py-3.5 md:text-lg  cursor-pointer hover:bg-[#e0e0e0] transition-all duration-200 font-semibold flex items-center  gap-5'>
        <FaFacebook size={30} />
